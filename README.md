### Hi there 👋, I'm Mia!
#### I'm an electrical/software engineer!
![Art of my cats](https://github.com/MichaByte/MichaByte/raw/main/cat_picture.png)

*I commissioned this beautiful art of my cats from [@TheColorfulKayak](https://www.instagram.com/TheColorfulKayak/)*

I think JS should be illegal 99% of the time (TS is the way!) 3D printing is awesome. I want to learn low level development. Linux > macOS > Windows (for programming at least)

Skills: KiCad // TypeScript // Svelte // SvelteKit // TailwindCSS // Python

- 🔭 I’m currently working on a PCB fidget toy. 
- 🌱 I’m currently learning KiCad.
- 👯 I’m looking to collaborate on just about anything! 
- 💬 Ask me about 3D printing! 
- 😄 Pronouns: she/her
- ⚡ Fun fact: I LOVE to build robots! 

![](https://dcbadge.limes.pink/api/shield/525432105224765494?logoColor=presence&theme=blurple)

[<img src='https://cdn.jsdelivr.net/npm/simple-icons@3.0.1/icons/github.svg' alt='github' height='40'>](https://github.com/miameows)  

PGP Fingerprint:
76CDF210584F7A3F2B361CFDABF0DDE643A9CAFA
